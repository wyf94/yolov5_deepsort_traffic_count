...
def callback_image(data):
    ...
    bridge = CvBridge()
    img_cv_rgb = bridge.imgmsg_to_cv2(data, "bgr8")

    img_cv_rgb = cv2.add(img_cv_rgb, polygon_color_image)
    ...
    img_cv_rgb = cv2.putText(img=img_cv_rgb, text=text_draw,
                                        org=draw_text_postion,
                                        fontFace=font_draw_number,
                                        fontScale=1, color=(0, 0, 255), thickness=2)

    #Display Image
    cv2.imshow("traffic count", img_cv_rgb)
	cv2.waitKey(1)
